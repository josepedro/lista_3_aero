...
for	x_i = 1:tamanhos(1)
	for	x_j = 1:tamanhos(2)
		for x_k = 1:tamanhos(3)
			estrutura_rho_vi_vj{x_i*x_j*x_k} = rho_vi_vj(x_i, x_j, x_k);
...
distancia = 0;
derivado_rho_vi_vj(1:tamanhos(1)*tamanhos(2)*tamanhos(3)) = 0;
disp('Calculando a bagaceira');
for ponto = 1:tamanhos(1)*tamanhos(2)*tamanhos(3)
	subtracao = matriz_distancia{1}-[15 15 15];
	elevando_quadrado = subtracao.^2;
	soma = sum(elevando_quadrado);
	distancia = sqrt(soma);
	derivado_rho_vi_vj(ponto) = estrutura_rho_vi_vj{ponto}/distancia;
end
...
laplaciano_rho_vi_vj = 	diff(diff(derivado_rho_vi_vj));
